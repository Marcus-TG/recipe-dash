...
export function productCode(
  raw: string,
  department: string | null,
): { code: string; kind: CodeKind } | null {
  const m = raw.match(/^\s*(\d{4,14})\s+(?=\S*[A-Za-z])/)
  if (!m) return null
  const code = m[1]!
  if (code.length >= 11) return { code, kind: 'upc' }
  // A short code is only a PLU under a produce header. Without that guard the
  // store's own address reads as produce: "4025 New Street" is PLU 4025,
  // Anjou pears.
  if (department === 'produce' && code.length <= 5 && lookupPlu(code)) {
    return { code, kind: 'plu' }
  }
  return { code, kind: 'sku' }
}
...
export function parseReceiptStructure(rawText: string): StructuredLine[] {
  ...
  for (const raw of rawText.split(/\r?\n/)) {
    ...
    if (RATE_LINE.test(line)) {
      ...
      if (weight && previous && previous.quantity == null) {
    ...
    const code = productCode(line, department)
    out.push({
      lineNo: out.length,
      rawText: line,
      code: code?.code ?? null,
